...
from transformer_train.transformer.tokenizer import SPECIAL_TOKENS, create_tokenizer
...
def test_tokenizer(tokenizer_dir=None):    
    ...
    chat_example = (
        ...
        "<|im_start|>assistant\nMerhaba! Ben iyiyim, teşekkürler. Sen nasılsın?<|im_end|>"
    )
    ...
    try:
        ...
        chat_tokens if isinstance(chat_tokens, list) and not (chat_tokens and isinstance(chat_tokens[0], list)) else (chat_tokens[0] if isinstance(chat_tokens, list) and chat_tokens and isinstance(chat_tokens[0], list) else chat_tokens)
        has_special_tokens = "<|im_start|>" in chat_decoded and "<|im_end|>" in chat_decoded
...
def interactive_test(tokenizer_dir=None):
    ...
    print("Try using special tokens like <|endoftext|>, <|im_start|>, <think>, etc.")
    print()
